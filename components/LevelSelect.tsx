...
const LevelSelect: FC<Props> = ({ level, onChange }) => {
    ...
    >
        ...
        <span className={classNames({
            'absolute top-[6px] right-[6px] transition-transform p-3': true,
            'rotate-180': active
        })}>
        ...
        {active
            ? <ul 
                className="absolute w-full shadow-md bg-white inset-x-0 rounded-xl top-[64px]"
            >
                <li className="p-4 pl-16 w-full text-lg relative cursor-pointer"
                    onClick={() => {
                        onChange(1)
                        setActive(false)
                    }}
